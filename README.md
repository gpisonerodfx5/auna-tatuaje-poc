...
11. [Costos operativos](#11-costos-operativos)
...
Pricing AWS `us-east-1` a abril 2026, lista pública. Detalle completo en [CLAUDE.md §15](CLAUDE.md#15-costos--modelo-operativo).

| Escenario (mensual) | Contactables | Min de voz | Costo USD | $ / contactable |
|---------------------|-------------:|----------:|----------:|----------------:|
| Media cohorte | 275 | 710 | 69 | 0,26 |
| **Base BASE_MARZO** | **548** | **1.420** | **130** | **0,24** |
| Doble cohorte | 1.100 | 2.841 | 250 | 0,23 |
| Campaña masiva (×5) | 2.740 | 7.103 | 611 | 0,23 |
| Extensión nacional (×10) | 5.500 | 14.207 | 1.214 | 0,22 |

**Driver dominante:** ~93% del costo es Nova Sonic 2 + Q in Connect + Connect voz + telco saliente. Infra base (Lambdas + DDB + SFN + SQS + Pipe) suma <USD 7/mes incluso a 10× volumen. La curva es prácticamente lineal.

---

## 12. Troubleshooting

### "Connect can not access Lex Bot alias" al hacer `associate-bot`
- Faltan los permisos `lex:CreateResourcePolicy` y similares. Ver [permisos](#6-permisos-iam-necesarios).
- El alias del bot debe apuntar a una versión publicada (`1`, `2`...), **no a `DRAFT`**.

### Nova Pro alucina fechas, repite el saludo en loop o usa la respuesta SYSTEM default
- Causa: el binding `orchestratorConfigurationList[Connect.SelfService]` no apunta al agent productivo.
- Fix: re-correr `scripts/deploy_qconnect.py` o `scripts/update_ai_agent.py`. El binding se valida automáticamente al final del script.
- También: si el Contact Flow tiene hardcoded un `x-amz-lex:q-in-connect:ai-agent-id=<id>:<version>` en `LexSessionAttributes`, este override gana sobre el binding global. Mantener el flow apuntando a la versión correcta.

### "Lex V1 is not supported by Nova Sonic"
- AWS partió Lex en V1 (`lex:*`) y V2 (`lexv2-models:*` + `lexv2-runtime:*`).
- Nova Sonic 2 sólo está disponible en Lex V2.
- Si el equipo de seguridad sólo te dio `lex:*`, pedir agregar `lexv2-models:*` y `lexv2-runtime:*` (son namespaces IAM separados — `lex:*` NO los cubre).

### Lambda devuelve `elegible=None` o el flow no llama a Lambda
- Verificar que la Lambda esté asociada al Connect instance **con el qualifier `:live`**:
  ```bash
  aws connect list-lambda-functions --instance-id <ID> --profile <PERFIL>
  ```
  Tiene que aparecer el ARN con `:live` al final. Si solo aparece sin qualifier, hacer:
  ```bash
  aws connect associate-lambda-function --instance-id <ID> \
    --function-arn arn:aws:lambda:us-east-1:<ACCT>:function:auna-tatuaje-poc-<fn>:live \
    --profile <PERFIL>
  ```

### CrearCita falla con `holderName`/`holderLastName` vacíos
- La Lambda tiene fallback: si recibe `holder_name`/`holder_last_name` vacíos, parsea `nombre_completo`.
- Si ambos llegan vacíos, devuelve error explícito. Verificar que el flow pase `holder_name` desde `$.Attributes.holder_name`.

### Timeout de Multisede UAT (~6s)
- El API a veces tarda 8-10s. La Lambda `crear-cita` tiene un fallback "asumir creada" con timeout de 6s.
- Si pasa frecuentemente, subir el timeout de la Lambda (default 30s) y el timeout de la llamada HTTP a Multisede.

Bugs históricos detallados en [CLAUDE.md §10](CLAUDE.md#10-bugs-resueltos--historial).
...
Para dudas técnicas sobre el repo, abrir un issue o contactar al equipo DFX5.
